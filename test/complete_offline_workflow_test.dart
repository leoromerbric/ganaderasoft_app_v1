...
void main() {
  ...
  group('Complete Offline Workflow Integration Tests', () {
    test(
      'complete offline workflow: create, edit, sync for animals and personal finca',
      () async {
        ...
        print(
          '✓ Animal synced: temp ID $animalTempId -> real ID $animalRealId',
        );
        ...
        allPending = await DatabaseService.getAllPendingRecords();
        final remainingUpdateRecords = allPending
            .where((r) => r['operation'] == 'UPDATE')
            .toList();
        ...
        print(
          '✓ Personal finca final state verified: ${finalPersonal['nombre']} ${finalPersonal['apellido']}',
        );
    ...
    test('multiple offline operations with mixed CREATE and UPDATE', () async {
      ...
      for (final personal in personalData) {
        await DatabaseService.savePendingPersonalFincaOffline(
          ...
          nombre: personal['nombre'] as String,
          apellido: personal['apellido'] as String,
          telefono: '300${personal['cedula']}',
          correo: '${personal['nombre'].toString().toLowerCase()}@test.com',
          tipoTrabajador: 'Operario',
        );
      }
      ...
      final creates = allPending
          .where((r) => r['operation'] == 'CREATE')
          .toList();
      final updates = allPending
          .where((r) => r['operation'] == 'UPDATE')
          .toList();
      ...
      print(
        '✓ Verified mixed operations: ${creates.length} CREATEs, ${updates.length} UPDATEs',
      );
      ...
      print(
        '✓ Verified type distribution: ${animalRecords.length} Animal, ${personalRecords.length} PersonalFinca',
      );
