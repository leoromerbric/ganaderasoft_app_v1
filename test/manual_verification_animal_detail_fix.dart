  ...
  try {
    ...
    if (tableNames.contains('animal_detail')) {
      ...
      try {
        ...
        await db.insert('animal_detail', testData, conflictAlgorithm: ConflictAlgorithm.replace);
        print('   ✓ INSERT operation successful');
        
        // Test query operation
        final result = await db.query('animal_detail', where: 'id_animal = ?', whereArgs: [999]);
        print('   ✓ SELECT operation successful (${result.length} rows)');
        
        // Test DatabaseService method
        final animalDetail = await DatabaseService.getAnimalDetailOffline(999);
        print('   ✓ DatabaseService.getAnimalDetailOffline() works');
        
        // Cleanup
        await db.delete('animal_detail', where: 'id_animal = ?', whereArgs: [999]);
