...
void main() {
  group('Password Hash Authentication Tests', () {
    ...
    test('should save and retrieve password hash', () async {
      ...
      const passwordHash = '48ff8a5b27ecef2b1b89cfaa4b3827c2c2fe395c7e7b50c1e9f8b93f77162e31'; // Example hash
      
      // Save user with password hash
      await DatabaseService.saveUserOffline(user, passwordHash: passwordHash);
      
      // Retrieve password hash
      final retrievedHash = await DatabaseService.getUserPasswordHash(user.email);
      
      expect(retrievedHash, equals(passwordHash));
    });

    test('should return null for non-existent user password hash', () async {
      final retrievedHash = await DatabaseService.getUserPasswordHash('nonexistent@example.com');
    ...
    test('should authenticate offline with correct password hash', () async {
      ...
      // First save user data (simulating successful online login)
      final authService = AuthService();
      ...
      const passwordHash = 'ef92b778bafe771e89245b89ecbc08a44a4e166c06659911881f383d4473e94f'; // SHA256 of 'myTestPassword123'
      await DatabaseService.saveUserOffline(user, passwordHash: passwordHash);
    ...
    test('should not authenticate offline with incorrect password', () async {
      ...
      const correctPassword = 'myTestPassword123';
      const wrongPassword = 'wrongPassword';
      
      // Save user with password hash
      const passwordHash = 'ef92b778bafe771e89245b89ecbc08a44a4e166c06659911881f383d4473e94f'; // SHA256 of correct password
      await DatabaseService.saveUserOffline(user, passwordHash: passwordHash);
    ...
    test('should handle case-insensitive email lookup for password hash', () async {
      final user = User(
        id: 1,
        name: 'Test User',
        email: 'Test@Example.com',
      ...
      final retrievedHash1 = await DatabaseService.getUserPasswordHash('test@example.com');
      final retrievedHash2 = await DatabaseService.getUserPasswordHash('TEST@EXAMPLE.COM');
      final retrievedHash3 = await DatabaseService.getUserPasswordHash('Test@Example.com');
    ...
    test('should update password hash when user is saved again', () async {
      ...
      await DatabaseService.saveUserOffline(user, passwordHash: oldPasswordHash);
      ...
      await DatabaseService.saveUserOffline(user, passwordHash: newPasswordHash);
