...
void main() {
  ...
  group('Issue #89 Sync Fix Tests', () {
    ...
    test('should handle personal finca sync correctly with new flow (no redundant save)', () async {
      ...
      print('Successfully called markPersonalFincaAsSynced: $tempId -> $realId');
      ...
      expect(finalPersonal.first['id_tecnico'], equals(realId)); // Should have real ID now
      expect(finalPersonal.first['synced'], equals(1)); // Should be marked as synced
      expect(finalPersonal.first['is_pending'], equals(0)); // Should not be pending
      expect(finalPersonal.first['pending_operation'], isNull); // Should clear operation
    ...
    test('should handle animal sync correctly (verification that animals work)', () async {
    ...
    test('should handle multiple pending records sync', () async {
      ...
      final finalPending = await DatabaseService.getPendingPersonalFincaOffline();
      expect(finalPending.length, equals(0));
      
      final db = await DatabaseService.database;
      final allPersonal = await db.query('personal_finca', orderBy: 'id_tecnico');
